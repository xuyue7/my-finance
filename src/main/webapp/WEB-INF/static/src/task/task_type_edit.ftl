							...
							<div class="layui-form" lay-filter="">
								<div class="layui-form-item">
									...
									<div class="layui-input-inline">
										<input type="hidden" name="taskTypeId" value="${entity.taskTypeId}">
										<input type="text" name="taskTitle" value="${entity.taskTitle}" lay-verify="taskTitle" class="layui-input" placeholder="请输入类型名称">
									</div>
								</div>
								<div class="layui-form-item">
									<label class="layui-form-label">类型描述<span style="color: red;">*</span></label>
									<div class="layui-input-inline" style="width:80%"><textarea name="taskBody" id="editor">${entity.taskBody}</textarea><!--<section id="editor"></section>--></div>
								</div>
					            <div class="layui-form-item">
		...
		<script>
			layui.use(['jquery', 'form', 'layer', 'upload','jquery'], function() {
				...
				form.on('submit(ff)', function(data) {
					$.ajax({
						type: 'post',
						url: '${BASE_PATH}/admin/task/ajax_task_type_edit',
						data: data.field,
