				...
				<div class="layui-col-md12">
					<div class="layui-card">
						...
						<div class="layui-inline">
								<button class="layui-btn layui-btn-primary" id="taskadd"><i class="layui-icon layui-icon-add-1" ></i> 新增问题</button>
								</div>
								<br />
								<br />
							<div class="layui-inline">
		...
		<script>
			layui.config({
				...
				request: {
					...
					url: '${BASE_PATH}/admin/operate/aqs_list',
					page: true,
					limit: 10,
				...
				//新增按钮
				 $("#taskadd").click(function(){
						layer.open({
							...
							title:'信息',
							area: ['70%', '90%'],
							content: "${BASE_PATH}/admin/operate/aqs_edit_page", //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['
						});
					})
				//监听查看按钮
				table.on('tool(test-table-operate)', function(obj) {
					...
					if(obj.event === 'detail') {
						layer.open({
							...
							title:'用户详细信息',
							area: ['70%', '90%'],
							content: "${BASE_PATH}/admin/operate/aqs_edit_page?aqsId=" + data.aqsId, //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['
						});
					...
					if(obj.event === 'del'){
						var index=layer.confirm('是否确认删除该信息？', {
							  btn: ['确认', '取消'] //可以无限个按钮
									...
									$.ajax({
									    type:'get',
									    url:'${BASE_PATH}/admin/operate/aqs_del',
									    data:{aqsId:data.aqsId},
