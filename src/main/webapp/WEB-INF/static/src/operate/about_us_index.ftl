				...
				<div class="layui-col-md12">
					<div class="layui-card">
						...
						<div class="layui-inline">
								<button class="layui-btn layui-btn-primary" id="taskadd"><i class="layui-icon layui-icon-add-1" ></i>新增关于我们</button>
								</div>
									<br />
										<br />
							<div class="layui-inline">
							...
							<script type="text/html" id="test-table-operate-barDemo">
								<a class="layui-btn layui-btn-xm" lay-event="detail">编辑</a>
								&nbsp;&nbsp;&nbsp;
  								<a class="layui-btn layui-btn-danger layui-btn-xm" lay-event="del">删除</a>
							</script>
						</div>
					</div>
				</div>
		...
		<script>
			layui.config({
				...
				request: {
					...
					url: '${BASE_PATH}/admin/operate/about_us_list',
					page: true,
					limit: 10,
				...
				table.on('tool(test-table-operate)', function(obj) {
					...
					if(obj.event === 'detail') {
						layer.open({
							...
							title:'信息',
							area: ['70%', '75%'],
							content: "${BASE_PATH}/admin/operate/about_us_edit_page?aboutUsId=" + data.aboutUsId, //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['
						});
					...
					if(obj.event === 'del') {
						var index=layer.confirm('是否确认删除该信息？', {
							  btn: ['确认', '取消'] //可以无限个按钮
									...
									$.ajax({
									    type:'get',
									    url:'${BASE_PATH}/admin/operate/about_us_del',
				...
				$("#taskadd").click(function(){
						layer.open({
							...
							title:'信息',
							area: ['70%', '75%'],
							content: "${BASE_PATH}/admin/operate/about_us_edit_page", //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['
						});
					})
				//搜索
				var $ = layui.$,
