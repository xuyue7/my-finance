							...
							<div class="layui-form" lay-filter="">
								...
								<div class="layui-form-item">
									...
									<div class="layui-input-block">
										...
										<input type="radio" title="禁用" [#if user.state = 1]checked[/#if] disabled="">
									</div>
								</div>
								<div class="layui-form-item">
									<label class="layui-form-label">实名认证<span style="color: red;">*</span></label>
									<div class="layui-input-block">
										<input type="radio" title="已认证" [#if user.applyState = 2]checked[/#if] disabled="">
										<input type="radio" title="未认证" [#if user.applyState != 2]checked[/#if] disabled="">
									</div>
								</div>
								[#if user.applyState = 0]
									<blockquote class="layui-elem-quote layui-quote-nm">
										用户${user.wxName}未进行实名认证！
									</blockquote>
								[/#if]
								[#if user.applyState = 1]
									<blockquote class="layui-elem-quote layui-quote-nm">
										用户${user.wxName}已提交实名认证,待审核！
									</blockquote>
								[/#if]
								[#if user.applyState = 3]
									<blockquote class="layui-elem-quote layui-quote-nm">
