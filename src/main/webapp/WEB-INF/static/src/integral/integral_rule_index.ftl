									...
									<div class="layui-input-inline">
										<div class="demoTable">
											...
											<div class="layui-inline">
												<input class="layui-input" id="demoReload" autocomplete="off" placeholder="请输入标题或者内容...">
											</div>
		...
		<script>
			layui.config({
				...
				var dicTable = table.render({
					...
					url: '${BASE_PATH}/admin/integral/integral_rule_data',
					page: true, //开启分页
					limit: 10,
				...
				table.on('tool(shop)', function(obj) {
					...
					if(obj.event === 'edit') {
						layer.open({
							...
							shade: 0.8,
							area: ['90%', '85%'],
							content: 'integral_edit?ruleId=' + ruleId
						});
					}
					if(obj.event === 'del') {
						var index = layer.confirm('您确认要删除吗？', {
						  btn: ['确认','取消'] //按钮
								...
								$.ajax({
				                   url:'${BASE_PATH}/admin/integral/integral_del',
				                   data:{ruleId:ruleId},
				...
				$("#add").click(function(){
					...
					shade: 0.8,
					area: ['90%', '85%'], 
					content: '${BASE_PATH}/admin/integral/add_rule_page' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
					}); 
				})
