...
<html>
	...
	<body>
		...
		<script type="text/html" id="test-table-operate-barDemo">
			<a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="open">获取积分详情</a>
		</script>
		<script>
			layui.config({
				...
				var dicTable = table.render({
					...
					url: '${BASE_PATH}/admin/integral/user_integral_data',
					page: true, //开启分页
					limit: 10,
				...
				table.on('tool(shop)', function(obj) {
					...
					if(obj.event === 'open') {
						layer.open({
							...
							shade: 0.8,
							area: ['90%', '85%'],
							content: '${BASE_PATH}/admin/integral/user_integral_page?uid=' + uid
						});
					}
				});
		...
		<script type="text/html" id="wxName">
			{{d.wxUser.wxName}}
		</script>
